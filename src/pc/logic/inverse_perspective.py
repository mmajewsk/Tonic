...
class InversePerspective(Action):
	def __init__(self, perspective_area=HAXOR_SRC, img_shape=(700, 700), desired_shape=(150,300), marigin=12):
		...
		self.roi = None
		self.destination = [
			(self.canvas_x_mid-self.img_x_mid, img_shape[1]-self.marigin-desired_shape[1]),
			(self.canvas_x_mid+self.img_x_mid, img_shape[1]-self.marigin-desired_shape[1]),
		...
		self.transformation = cv2.getPerspectiveTransform(self.perspective_area, self.destination)
